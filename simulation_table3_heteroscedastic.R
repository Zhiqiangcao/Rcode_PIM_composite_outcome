#This function is to reproduce simulation results of normal linear regression with Heteroscedastic in Tables 3 of paper
...
gendata = function(n,alpha,u,sig,aa,bb){
  ...
  for(i in 1:n){
    ...
    mv.NE <- mvdc(normalCopula(0.75), c("norm", "norm"),
                  list(list(mean = 0, sd =sigi), list(mean = 0, sd = sigi)))
    x.sample = rMvdc(1, mv.NE)
    D[i] = y[i]+x.sample[1]
    H[i] = y[i]+x.sample[2]
  }
...
censor_choice = function(case,choice){
  ...
  if(case == 1){
    alpha = 1; u = 1; sig = 1; 
    if(choice==1){aa = 0.01; bb = 3.3} 
    if(choice==2){aa = 500; bb = 1000}
  }else if(case == 2){
    alpha = 1; u = 1; sig = 5; 
    if(choice==1){aa = 1.5; bb = 6.0} 
    if(choice==2){aa = 500; bb = 1000}
  }else if(case == 3){
    alpha = 1; u = 10; sig = 1;
    if(choice==1){aa = 6.0; bb = 10.2} 
    if(choice==2){aa = 500; bb = 1000}
  }else if(case == 4){
    alpha = 1; u = 10; sig = 5;
    if(choice==1){aa = 12; bb = 15} 
    if(choice==2){aa = 500; bb = 1000}
  }else if(case == 5){
    alpha = 10; u = 1; sig = 5;
    if(choice==1){aa = 8; bb = 11} 
    if(choice==2){aa = 500; bb = 1000}
  }
...
for(case in case_set){
  ...
  bb = cc_res$bb
  #true coefficients
  beta = alpha/(sig)  #relationship between alpha and beta
  ...
  for(i in 1:N){
    ...
    status <- gdata$status
    Z <- cbind(z1=gdata$z)
    res = pim.co(ID, time, status, Z, heter = TRUE, link="probit", start=start, method="Newton")
  ...
  res = round(resu,3)
  write.csv(res,paste0("case_",case,"choice_",choice,"_n_",n,"lm","heter",".csv"))
  cat("case=",case,"\n")
}
